...
const Header = () => {
  ...
  // lock body scroll properly for Mobile
  useEffect(()=> {
    if(menuOpen){
  ...
  return (
    < >
      {/* header */}
      <header className={`fixed top-0 left-0 z-50 w-full py-4 lg:px-[100px] px-7
      ...
      <div className='flex justify-between items-center'>
        ...
        <div className='text-orange-600 font-bold lg:text-4xl text-3xl'>
          Tripzen <span className='text-white'>.</span>
        </div>
      ...
      {/* mobile fullscreen menu /(vertical menu side) */}
      <div className={`fixed inset-0 bg-black text-white z-[55]
      transform transition-transform duration-300 ease-out will-change-transform
      ${menuOpen ? 'translate-y-0' : '-translate-y-full'} flex items-center justify-center
      overscroll-none`}>
